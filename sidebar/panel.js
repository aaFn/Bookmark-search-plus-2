...
// ####
let lastSelectedBN_id ; // global variable to store the last selected BN id
let retainlastselectedbook_option = false ; // Boolean

/*
 * Save the last selected BN_id
 */
...
// ####
  ...
  // trace("clickHandler | target:"+target.nodeName.toLowerCase()+" curRow"+curRow ) ;
  if ( curRow != undefined ) {
    lastSelectedBN_id = curRow.dataset.id ;
    saveLastSelectedBN_id( lastSelectedBN_id ) ;
  }
// ####
...
// ####
    ...
    if ((value = res.retainlastselectedbook_option) != undefined) {
      ...
      // Cleaning of local store old version .. delete on long term (2.0.30+)
      if (retainlastselectedbook_option == "true") {
        retainlastselectedbook_option = true;
